...
def scrambling_sampling_1d(img,phi,perm):
    ...
    # # 对数归一化（适合DCT系数）
    # np_dct = dct_img[0][0].cpu().detach().numpy()
    # np_dct = np.log(np.abs(np_dct) + 1e-6)
    # np_dct = (np_dct - np.min(np_dct)) / (np.max(np_dct) - np.min(np_dct) + 1e-6)
    # np_dct = (np_dct * 255).astype(np.uint8)
    # cv2.imshow('np_dct', (np_dct))
    ...
    # # 对数归一化（适合DCT系数）
    # np_scr = scr_img.reshape(h,w).cpu().detach().numpy()
    # np_scr = np.log(np.abs(np_scr) + 1e-6)
    # np_scr = (np_scr - np.min(np_scr)) / (np.max(np_scr) - np.min(np_scr) + 1e-6)
    # np_scr = (np_scr * 255).astype(np.uint8)
    # cv2.imshow('np_scr', (np_scr))
...
if __name__ == '__main__':
    ...
    img = cv2.imread('G:\date\cs_data\Set11/Parrots.tif',0)
    ...
    checkpoint_model = torch.load('model_epoch_gray.pth')
